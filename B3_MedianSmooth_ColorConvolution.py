# Nama File : SmoothingMedian.py
# Deskripsi : Algoritma pemrosesan Citra digital untuk menerapkan smoothing median ke Citra berwarna
    ...
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            for k in range(image.shape[2]):  # Untuk setiap channel warna
                ...
                # Hitung median
                output[i, j, k] = np.median(neighborhood)
...
hasil_median1 = medianFilter(noisy1, 3)
hasil_median2 = medianFilter(noisy2, 3)

# Gabungkan gambar untuk masing-masing citra
gabung1 = np.hstack((citra1, noisy1, hasil_median1))
gabung2 = np.hstack((citra2, noisy2, hasil_median2))
...
for gabung, width in [(gabung1, citra1.shape[1]), (gabung2, citra2.shape[1])]:
    ...
    cv2.putText(gabung, "Noisy (10%)", (width + 10, 30), 
               cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    cv2.putText(gabung, "Median 3x3", (2*width + 10, 30), 
               cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

# Tampilkan hasil dalam window terpisah
cv2.namedWindow('Hasil Median Filter Lena', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Hasil Median Filter Lena', 1200, 400)
cv2.imshow('Hasil Median Filter Lena', gabung1)

cv2.namedWindow('Hasil Median Filter Tree', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Hasil Median Filter Tree', 1200, 400)
cv2.imshow('Hasil Median Filter Tree', gabung2)
